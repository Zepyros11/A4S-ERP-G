...
### ✅ แก้แล้วบางส่วน 30 ก.ค. 2569 — กุญแจถอดรหัสสมาชิกไม่อยู่ในฐานข้อมูลอีก

เดิม `app_settings.member_master_key` อ่านได้ด้วย anon key = ใครก็ดึงกุญแจไป
ถอดข้อมูลส่วนตัวสมาชิก 117,000 คนได้โดยไม่ต้อง login

**ตอนนี้กุญแจอยู่ใน env `MEMBER_MASTER_KEY` ของ ai-proxy เท่านั้น**
ขอผ่าน `POST /master-key` ซึ่งต้องผ่าน 3 ด่าน:
รหัสผ่าน ERP ถูก (SHA-256) → บัญชียัง active → มีสิทธิ์ `member_decrypt`
· ยิงผิด 5 ครั้ง ล็อก 10 นาที · ข้อความ error ไม่บอกใบ้ว่ามี username นั้นจริงไหม

**ผู้ใช้ไม่ต้องทำอะไร** — [login.html](../login.html) ดึงกุญแจให้ตอน login (จุดเดียวที่มีรหัสผ่านอยู่ในมือ)
ถ้าพลาด (proxy หลับ · timeout 6 วิ) → `ERPCrypto.ensureMasterKey()` ใน
[js/core/crypto.js](../js/core/crypto.js) จะถามรหัสผ่านเป็นทางสำรอง

> ⚠️ ยังมีอีก ~13 จุดในโค้ดที่เช็ค `hasMasterKey()` แล้วขึ้น "ต้องตั้ง Master Key ก่อน" เฉยๆ
> ไม่ได้แก้ให้ขอกุญแจเอง — ถ้าเจอ **ให้ logout แล้ว login ใหม่** จะได้กุญแจมาเอง
> (ที่แก้แล้ว: members-list · attendees · daily-sale · members-sync · dev-tool settings)

**ที่ยังไม่ได้แก้** — ตารางอื่นยังอ่าน/เขียนได้ด้วย anon key ตามข้อ 1 ด้านล่าง
...
### 🟠 2. โควต้า LINE 300 ข้อความ/เดือน — น้อยเกินไป

ตรวจเมื่อ 30 ก.ค. 2569: **ใช้ครบ 300/300 แล้วทั้งที่ยังไม่สิ้นเดือน**
→ แจ้งเตือนอัตโนมัติที่ควรส่งถึงพนักงาน **หายไปเงียบๆ โดยไม่มีใครรู้**

เช็คได้ที่ `GET https://<proxy>/line/quota`

| สิ่งที่กินโควต้า | สิ่งที่ไม่กิน |
|---|---|
| push · multicast · broadcast (cron แจ้งเตือน, LINE promote) | **reply** (ตอบกลับในแชท — ฟรีไม่จำกัด) |

ทางเลือก: อัปเกรดแพ็กเกจ LINE OA · ลดจำนวนแจ้งเตือน · ย้ายบางอย่างไปใช้กระดิ่งในเว็บ (มีอยู่แล้ว)
...
### 🟠 4. รหัสผ่านพนักงานใช้ SHA-256 เปล่า
