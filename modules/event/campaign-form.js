...
let rewardMeta = {
  mode: "ranked", metric: ["likes"], combine_channels: false, announce_date: null,
  channels: {
...
function fillForm(camp) {
  ...
  const annEl = document.getElementById("rwAnnounceDate");
  if (annEl) annEl.value = rewardMeta.announce_date || "";

  // ภาพสถานะแคมเปญ — default = ไฟล์ใน asset ถ้ายังไม่เคยอัปเอง
  const rw = camp?.rewards || {};
  notOpenMgr.setItems([{ url: rw.not_open_image || DEFAULT_NOT_OPEN_IMG, type: "image", name: "" }]);
  endedMgr.setItems([{ url: rw.ended_image || DEFAULT_ENDED_IMG, type: "image", name: "" }]);
  announceMgr.setItems(
    (Array.isArray(rw.announce_images) ? rw.announce_images : [])
      .map((m) => (typeof m === "string" ? m : m && m.url))
      .filter(Boolean)
      .map((u) => ({ url: u, type: "image", name: "" })),
  );
...
// ภาพสถานะแคมเปญ (public): ปก "ยังไม่เปิด" / "จบแล้ว" (1 ภาพ) + ประกาศผลรางวัล (≤10)
const DEFAULT_NOT_OPEN_IMG = "../../assets/images/campaign-not-open.png";
const DEFAULT_ENDED_IMG = "../../assets/images/campaign-ended.png";
const notOpenMgr = createMediaManager({
  gridId: "fNotOpenGrid", fileInputId: "fNotOpenFiles", countId: "fNotOpenCount",
  max: 1, imagesOnly: true, allowCover: false, emptyLabel: "เพิ่มปก",
});
const endedMgr = createMediaManager({
  gridId: "fEndedGrid", fileInputId: "fEndedFiles", countId: "fEndedCount",
  max: 1, imagesOnly: true, allowCover: false, emptyLabel: "เพิ่มปก",
});
const announceMgr = createMediaManager({
  gridId: "fAnnounceGrid", fileInputId: "fAnnounceFiles", countId: "fAnnounceCount",
  max: 10, imagesOnly: true, allowCover: false, emptyLabel: "เพิ่มรูป",
});
...
function normalizeRewards(raw, rankMetric) {
  ...
  const out = { mode: "ranked", metric: ["likes"], combine_channels: false, announce_date: null, channels: emptyChannels() };
  ...
  out.announce_date = (raw.announce_date || "").slice(0, 10) || null;
...
function collectRewards() {
  ...
  const announceDate = (document.getElementById("rwAnnounceDate")?.value || "").trim() || null;
  return { mode: rewardMeta.mode, metric: rewardMeta.metric, combine_channels: !!rewardMeta.combine_channels, announce_date: announceDate, channels };
}
...
window.saveCampaign = async function () {
  ...
  try {
    ...
    // upload ภาพสถานะแคมเปญ (public): not-open / ended / ประกาศผลรางวัล
    const uploadStateImg = async (mgr, prefix) => {
      const items = mgr.getItems();
      if (!items.length) return null;
      const m = items[0];
      if (m.url) return m.url; // url เดิม หรือ default asset — เก็บตามนั้น
      const safe = (m.name || "img").replace(/[^a-zA-Z0-9._-]/g, "_");
      const path = `campaigns/${token}/${prefix}_${Date.now()}_${safe}`;
      const u = await window.ImageCompressor.uploadViaRest(url, key, BUCKET, path, m.file);
      if (!u) throw new Error(`อัปโหลดภาพ ${prefix} ไม่สำเร็จ`);
      return u;
    };
    const notOpenImage = await uploadStateImg(notOpenMgr, "notopen");
    const endedImage = await uploadStateImg(endedMgr, "ended");
    const announceImages = [];
    const annItems = announceMgr.getItems();
    for (let i = 0; i < annItems.length; i++) {
      const m = annItems[i];
      if (m.url) { announceImages.push(m.url); continue; }
      const safe = (m.name || "img").replace(/[^a-zA-Z0-9._-]/g, "_");
      const path = `campaigns/${token}/announce_${Date.now()}_${i}_${safe}`;
      const u = await window.ImageCompressor.uploadViaRest(url, key, BUCKET, path, m.file);
      if (!u) throw new Error("อัปโหลดภาพประกาศผลรางวัลไม่สำเร็จ");
      announceImages.push(u);
    }
    const rewards = collectRewards();
    rewards.not_open_image = notOpenImage;
    rewards.ended_image = endedImage;
    rewards.announce_images = announceImages;
