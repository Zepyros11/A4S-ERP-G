...
async function loadCaretakers() {
  ...
  // — ไม่นับ Admin/BRE/CS แม้จะมี IBD เป็น role เสริม (เพราะไม่ได้อยู่แผนก IBD โดยตรง)
...
window.updateCaretaker = async function (id, sel) {
  ...
  try {
    await sbFetch(`ibd_relocation_requests?id=eq.${id}`, { method: 'PATCH', body: JSON.stringify({ caretaker_user_id: v }) });
