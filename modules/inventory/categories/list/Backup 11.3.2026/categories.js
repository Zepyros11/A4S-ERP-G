...
function addSegment() {
  ...
  inputs.forEach((el, idx) => {
    ...
    if (typeof skuSegments[idx] === "string") {
      skuSegments[idx] = {
        label: val,
        locked: false,
      };
    } else {
      skuSegments[idx].label = val;
    }
  });

  if (skuSegments.length >= 6) {
    showToast("เพิ่มได้สูงสุด 6 ช่อง", "warning");
    return;
  }
...
function updateSkuPreview() {
  ...
  const prefix = (
    document.getElementById("skuF1")?.value || "XXX"
  ).toUpperCase();
  ...
  const segExample = skuSegments
    .map((s) => {
      ...
      return (label || "X").substring(0, 4).toUpperCase();
    })
    .join("-");
...
function toggleLock(i) {
  ...
  if (typeof skuSegments[i] === "string") {
    skuSegments[i] = {
      label: skuSegments[i],
      locked: true,
    };
...
function renderSegments() {
  ...
  skuSegments.forEach((seg, i) => {
    ...
    const locked = typeof seg === "object" ? seg.locked : false;
...
function openModal(data = null) {
  ...
  skuSegments =
    labels.segments && labels.segments.length
