...
function renderVariantSkuRows() {
  ...
  for (let i = 0; i < 5; i++) {
    const el = document.getElementById(`sku_s${i}`);
    if (!el) continue;
    const segLabel = segments[i]?.label || `ช่อง ${i + 2}`;
    allSegments.push({ value: el.value || "?", label: segLabel });
  }

  const lockedSegments = allSegments.slice(0, -1);
  const variantSegment = allSegments[allSegments.length - 1];
  const variantLabel =
    segments[allSegments.length]?.label || variantSegment?.label || "ตัวเลือก";
  ...
  if (state.variants.length === 0) {
    container.innerHTML = `<div style="text-align:center;padding:20px;color:var(--text3);
      ...
      กด "+ เพิ่มตัวเลือก" เพื่อเพิ่ม SKU ย่อย เช่น สีแดง, ไซส์ S</div>`;
    updateSummary();
    return;
  }
  ...
  container.innerHTML = state.variants
    .map((v, vi) => {
      ...
      const lockedCols = lockedSegments
        .map(
          (seg) => `
      <div style="flex:1;min-width:0;">
        <div style="font-size:11px;font-weight:700;color:var(--text3);margin-bottom:3px;">${seg.label}</div>
      ...
      return `<div style="background:var(--surface2);border:1.5px solid var(--border);
      ...
      <div style="display:flex;gap:10px;align-items:flex-end;margin-bottom:8px;">
        ...
        ${lockedCols}
        <div style="flex:1;min-width:0;">
          <div style="font-size:11px;font-weight:700;color:var(--accent);margin-bottom:3px;">${variantLabel}</div>
          <input class="form-control variant-label-input" value="${v.variantLabel}" placeholder="เช่น RED, S"
...
function updateVariantSkuLabel(vi, val) {
  ...
  for (let i = 0; i < 5; i++) {
    const el = document.getElementById(`sku_s${i}`);
    if (el) allSegs.push(el.value || "?");
  }
  const lockedSegs = allSegs.slice(0, -1);
  const vSeq =
    state.variants[vi].seq || document.getElementById("sku_seq")?.value || "?";
  const label = val.trim().toUpperCase() || "?";
  state.variants[vi].sku = [f1, ...lockedSegs, label, vSeq]
    .join("-")
    .toUpperCase();
  ...
  try {
    ...
    if (state.productType === "variable") {
      ...
      state.variants.forEach((v, vi) => {
        if (variantInputs[vi]) {
          ...
          if (label) {
            ...
            state.variants[vi].sku = [f1, ...lockedSegs, label, v.seq || seq]
              .join("-")
              .toUpperCase();
          }
        }
      ...
      const parentVariantLabel = (
        allSegs[allSegs.length - 1] || ""
      ).toUpperCase();
      const parentSku = [f1, ...lockedSegs, parentVariantLabel, seq]
        .join("-")
        .toUpperCase();
